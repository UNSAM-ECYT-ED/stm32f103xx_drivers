...
ARM_TOOLCHAIN_EXTRACT_DIR_NAME="${ARM_TOOLCHAIN_FILENAME%.*}"
...
curl -L ${ARM_TOOLCHAIN_URL} --create-dirs \
...
if grep -Fxq "$${HOME}/bin" ${HOME}/.bashrc
then
...
echo "Installation successful! close the terminal and open a new one!"
